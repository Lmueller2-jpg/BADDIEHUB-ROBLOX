...
-- Compatibility helper to wrap any tab control methods dynamically
local function CreateTab(title, icon)
    ...
    if tab then
        if not tab.Button and tab.AddButton then tab.Button = tab.AddButton end
        if not tab.AddButton and tab.Button then tab.AddButton = tab.Button end
        
        if not tab.Toggle and tab.AddToggle then tab.Toggle = tab.AddToggle end
        if not tab.AddToggle and tab.Toggle then tab.AddToggle = tab.Toggle end
        
        if not tab.Slider and tab.AddSlider then tab.Slider = tab.AddSlider end
        if not tab.AddSlider and tab.Slider then tab.AddSlider = tab.Slider end
        
        if not tab.Dropdown and tab.AddDropdown then tab.Dropdown = tab.AddDropdown end
        if not tab.AddDropdown and tab.Dropdown then tab.AddDropdown = tab.Dropdown end
        
        if not tab.TextBox and tab.AddTextbox then tab.TextBox = tab.AddTextbox end
        if not tab.AddTextbox and tab.TextBox then tab.AddTextbox = tab.TextBox end
        if not tab.TextBox and tab.AddTextBox then tab.TextBox = tab.AddTextBox end
        if not tab.AddTextBox and tab.TextBox then tab.AddTextBox = tab.TextBox end
        
        if not tab.Keybind and tab.AddKeybind then tab.Keybind = tab.AddKeybind end
        if not tab.AddKeybind and tab.Keybind then tab.AddKeybind = tab.Keybind end
        if not tab.Keybind and tab.AddBind then tab.Keybind = tab.AddBind end
        if not tab.AddBind and tab.Keybind then tab.AddBind = tab.Keybind end

        if not tab.Section and tab.AddSection then tab.Section = tab.AddSection end
        if not tab.AddSection and tab.Section then tab.AddSection = tab.Section end
    end
    return tab
end
